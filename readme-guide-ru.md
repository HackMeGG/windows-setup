...
#### 1. <code style="color : gold">Отключаем автоматическое обновления драйверов (по желанию):</code>
...
◻️ Совет: Режим "Сон" или "Гибернация" не перезагружает систему и не очищает данные в памяти поэтому стоит их отключить.\_
...
#### 12. Отключение Windows Copilot (по желанию):
...
1. _Быстрый способ_: скачать и запустить файл [disable_mpo с сайта Nvidia](https://nvidia.custhelp.com/app/answers/detail/a_id/5157)

2. _Второй способ, вручную_: WIN+R → regedit → **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\Dwm** → Создаём параметр DWORD32бит с названием **OverlayTestMode** и со значением **5**.

#### 14. Отключение "VBS (Изоляция ядра, Целостность памяти)":
...
#### 15. Настройка сетевого адаптера:

- Диспетчер устройств → Ваш сетевой адаптер →
  - Управление питанием → Снять все галочки.
  - Отключите во вкладке _Advanced_ → **Energy-Efficient Ethernet; Gigabit Lite; Green Ethernet; Power Saving Mode; Всё где есть Wake on magic; Shutdown Wake-On-Lan**.

#### 16. Переключение видеокарту в режиме MSI:
...
#### 18. Автоматический перезапуск приложений при запуске системы:
...
#### 20. Отключаем авто-обновление приложений из магазина Microsoft Store:
...
#### 21. Отключение залипание клавиш:
...
#### 22. Отключение Hyper-V (по желанию):
...
#### 23. Классическое контекстное меню ПКМ в Windows 11 & отключить задержку показа меню (по желанию):
...
#### 24. Остановка слежки Windows (по желанию):
...
#### 26. Отключить ненужные приложения из автозагрузки.
